...
// Modified by Matt Blair, June 2015
...
class Earcut {
public:
    using Vertex = std::array<Coord, 3>;
...
N Earcut<Coord, N>::linkedList(const Ring &points,
    ...
    for (i = 0, j = len - 1; i < len; j = i++) {
        p1 = points[i];
        p2 = points[j];
        const auto p20 = p2[0]; //util::nth<0, Point>::get(p2);
        const auto p10 = p1[0]; //util::nth<0, Point>::get(p1);
        const auto p11 = p1[1]; //util::nth<1, Point>::get(p1);
        const auto p21 = p2[1]; //util::nth<1, Point>::get(p2);
        sum += (p20 - p10) * (p11 + p21);
    }
...
N Earcut<Coord, N>::createNode(const Point &p) {
    ...
    assert(used.size() == i);
    nodes.emplace_back();
    vertices.emplace_back(Vertex {{ Coord(p[0]),
                                    Coord(p[1]) }});
