...
static TESSalloc default_allocator = {&alloc, &realloc, &free, nullptr,
...
void Builders::buildPolygon(const Polygon& _polygon, PolygonOutput& _out, MemPool* pool) {

    TESSalloc allocator(default_allocator);

    if (pool != nullptr) {
        allocator.memalloc = &pool_alloc;
        allocator.memfree = &pool_free;
        allocator.userData = pool;
    }
